...
export async function GET() {
  try {
    ...
    try {
      // Get account balances using the JWT authentication
      const accountsResponse = await getAccounts()
      const accounts = accountsResponse.accounts || []

      console.log("Successfully fetched account balances, fetching open orders")

      // Get open orders using the JWT authentication
      const ordersResponse = await getOpenOrders()
      const openOrders = ordersResponse.orders || []

      console.log("Successfully fetched open orders, processing positions")
    ...
    return NextResponse.json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Unknown error fetching positions",
        timestamp: new Date().toISOString(),
      },
      { status: 500 },
    )
